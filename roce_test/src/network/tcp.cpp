...
void TCP::send_vector(const std::vector<double>& data, int ip){
   mutx.lock();
   size_t data_size = data.size() * sizeof(double);
   write(tcp->clnt_socks[ip], data.data(), data_size);
   mutx.unlock();
}
int TCP::recv_vector(int ip,std::vector<double>& recv,double buf_size){
   //char msg[100];
   size_t data_size = recv.size() * sizeof(double);
   int str_len;
   str_len=read(tcp->new_sock[ip], recv.data(), data_size);
   //cout << str_len << endl;
   return str_len;
}
